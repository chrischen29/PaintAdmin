...
# --- 2. Google Sheets 服務初始化 ---
def get_sheets_service():
    scope = ['https://www.googleapis.com/auth/spreadsheets']
    
    # 從環境變數讀取 JSON 字串 (Key 為 GOOGLE_CREDS_JSON)
    ...
    if creds_json:
        try:
            ...
            raise Exception(f"解析環境變數金鑰失敗: {e}")
    else:
        # 備用方案：讀取本地檔案 (如果你之後有成功 push 檔案)
        creds_path = os.path.join(os.path.dirname(__file__), 'credentials.json')
        if os.path.exists(creds_path):
            creds = service_account.Credentials.from_service_account_file(creds_path, scopes=scope)
            return build('sheets', 'v4', credentials=creds)
        else:
            raise FileNotFoundError("找不到金鑰：請在 Render 設定 GOOGLE_CREDS_JSON 環境變數")
...
def admin():
    ...
    try:
        ...
        return f"系統連線錯誤: {str(e)}。請確認 Render 的 Environment 已經設定 GOOGLE_CREDS_JSON。"
